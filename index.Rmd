...
In cluster 3, the countries, on average, contribute close to 2.5% of total world trade, much lower than in cluster 2 but also much higher than cluster 1 which contains most of the r funccountries in the world but each contributed negligibly in comparison to world trade. 
...
Looking at the distribution of trade worldwide, we see that China, the United States, and Germany appear to dominate both with respect to exports and imports. China is by far responsible for the most exports, while the US is responsible for the most imports. Though in our initial maps, the rest of the world all looked fairly similar, we were actually able to identify three natural groups of countries based on trade volume -- high, medium, and low. As expected, China, Germany, and the United States are the sole members of the high trade volume category, with export and import volume far higher than the rest. 

Examining the trade relationships within clusters, we see a strong flow of goods and services from China to the United States -- by far the strongest relationship we identify overall. Even in the medium trade flow cluster, we are able to identify some strong relationships. Netherlands, Belgium-Luxembourg, France, and the UK seem to have high trade flow between them, as do Singapore and Malaysia. 
...
# Citations (need to add in properly) 
