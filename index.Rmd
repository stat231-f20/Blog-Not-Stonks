...
Through grouping by exporter and importer and summing the value of trade, we are able to calculate trade volumes (in terms of value, for better comparison) between various different countries.
...
From this graph of average distance to centroid against cluster number, we can see that there is a clear "hook" at k=3, meaning that our k-means method does not reduce much more variation in distance to centroid after k=3. Thus, the best cluster size for k-means is 3. 
...
From the graph above, we can see the large disparity existing between each cluster. The three countries in cluster 2 contributed significantly more to world exports than the rest of the other countries as they on average contribute to 10% to world trade individually. In cluster 3, the countries do contribute on average close to 2.5% of total world trade, much lower than in cluster 2 but also much higher than cluster 1 which contains most of the countries in the world but each contributed very little to world trade. We want to further see what the relationship is between countries in cluster 2 and 3.
