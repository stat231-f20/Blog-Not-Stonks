...
# Use k-means clustering to identify 4 clusters based on contribution in terms of export and import to world trade
set.seed(100)
vars <- c("export_pct" , "import_pct")
km_trade <- kmeans(trade_combine[,vars], centers=4, nstart=20)

trade_combine_clust4 <- trade_combine %>%
  mutate(clust4 = as.character(km_trade$cluster))

# visualize the cluster assignments and centroids
ggplot(data = trade_combine_clust4, aes(x = import_pct, y = export_pct)) + 
  geom_point(aes(color = clust4)) +
  coord_fixed() +
...
trade_network_clust <- trade_network %>%
  left_join(trade_combine_clust4, by = c("exporter_name" = "country")) %>%
  rename(clust_ex = clust4) %>%
  select(exporter_name, importer_name, total_trade, clust_ex) %>%
  left_join(trade_combine_clust4, by = c("importer_name" = "country")) %>%
  rename(clust_im = clust4) %>%
...
# See network within cluster 4
clust_4_network <- trade_network_clust %>%
  filter(clust_ex == 4) %>%
  filter(clust_im == 4)


clust_4_network_data <- graph_from_data_frame(clust_4_network
                                   , directed = TRUE)

clust_4_network_graph <- ggnetwork(clust_4_network_data)

ggplot(data = clust_4_network_graph
       , aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(aes(color = total_trade), arrow = arrow(type = "closed", length = unit(6,"pt")), 
             curvature = 0.1, size = 0.8)  +
  geom_nodes() +
  geom_nodelabel(aes(label = name)) +
  theme_blank() +
  labs(title = "Network for countries in cluster 4", 
       color = "2017 trade volume") +
  scale_color_continuous(type = "viridis")
```
